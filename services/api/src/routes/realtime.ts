  ...
  wss.on('connection', async (clientWs: WebSocket) => {
    ...
    try {
      ...
      azureWs.on('error', (err) => {
        ...
        if (clientWs.readyState === WebSocket.OPEN) {
          clientWs.send(JSON.stringify({ type: 'error', error: { message: 'Upstream connection error' } }));
      ...
      console.error('[relay] Failed to establish Azure connection:', err);
      if (clientWs.readyState === WebSocket.OPEN) {
        clientWs.send(JSON.stringify({ type: 'error', error: { message: 'Failed to connect to Azure OpenAI' } }));
